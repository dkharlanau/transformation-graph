...
from .linking import apply_links, validate_links
...
def load_project_manifest(path: str | Path) -> tuple[Path, dict[str, Any]]:
    ...
    for index, item in enumerate(adapters):
        ...
        if item.get("kind") not in ADAPTER_KINDS:
            raise GraphValidationError(f"{source}: sources.adapters[{index}].kind must be mapping, interface, or process")
    ...
    validate_links(raw.get("links", []), location=f"{source}: links")
...
def build_project(manifest_path: str | Path, output_dir: str | Path, *, base_url_override: str | None = None, force_site: bool | None = None) -> dict[str, Any]:
    ...
    links = validate_links(manifest.get("links", []), location="links")
    ...
    report: dict[str, Any] = {
        ...
        "links": {"count": len(links), "items": links},
        "graph": graph.stats(),
