...
from .contracts import ADAPTER_CONTRACT_ID, ADAPTER_CONTRACT_VERSION
...
def _outgoing(graph: Graph, node_id: str, relation: str | None = None) -> list[Edge]:
    return sorted([edge for edge in graph.edges if edge.source == node_id and (relation is None or edge.type == relation)], key=lambda edge: (edge.type, edge.target, edge.label or ""))


def _incoming(graph: Graph, node_id: str, relation: str | None = None) -> list[Edge]:
    return sorted([edge for edge in graph.edges if edge.target == node_id and (relation is None or edge.type == relation)], key=lambda edge: (edge.type, edge.source, edge.label or ""))
...
def _mapping_conformance(graph: Graph, findings: list[dict[str, str]]) -> None:
    ...
    if not mappings:
        findings.append(_finding("MAPPING_NODE_MISSING", "error", "adapter output contains no mapping node")); return
    for mapping_id in mappings:
        if not _targets_of_type(graph, _outgoing(graph, mapping_id, "maps_from"), "data_object"):
            findings.append(_finding("MAPPING_SOURCE_MISSING", "error", "mapping has no maps_from data object", mapping_id))
        if not _targets_of_type(graph, _outgoing(graph, mapping_id, "maps_to"), "data_object"):
        ...
        for rule_id in rules:
            if not _targets_of_type(graph, _outgoing(graph, rule_id, "reads"), "field"):
                findings.append(_finding("RULE_SOURCE_FIELD_MISSING", "error", "field rule reads no source field", rule_id))
            if not _targets_of_type(graph, _outgoing(graph, rule_id, "writes"), "field"):
...
def _interface_conformance(graph: Graph, findings: list[dict[str, str]]) -> None:
    ...
    if not interfaces:
        findings.append(_finding("INTERFACE_NODE_MISSING", "error", "adapter output contains no interface node")); return
    for interface_id in interfaces:
        if not _targets_of_type(graph, _outgoing(graph, interface_id, "sourced_from"), "system"):
            findings.append(_finding("INTERFACE_SOURCE_SYSTEM_MISSING", "error", "interface has no source system", interface_id))
        if not _targets_of_type(graph, _outgoing(graph, interface_id, "delivers_to"), "system"):
            findings.append(_finding("INTERFACE_TARGET_SYSTEM_MISSING", "error", "interface has no target system", interface_id))
        if not _targets_of_type(graph, _outgoing(graph, interface_id, "owned_by"), "owner"):
            findings.append(_finding("INTERFACE_OWNER_MISSING", "warning", "interface has no ownership relationship", interface_id))
        if not _sources_of_type(graph, _incoming(graph, interface_id, "validates"), "test"):
    ...
    while frontier:
        ...
        for edge in _outgoing(graph, current, "precedes"):
            if edge.target in step_ids and edge.target not in seen:
                seen.add(edge.target); frontier.append(edge.target)
    return seen
...
def _process_conformance(graph: Graph, findings: list[dict[str, str]]) -> None:
    ...
    if not processes:
        findings.append(_finding("PROCESS_NODE_MISSING", "error", "adapter output contains no process node")); return
    for process_id in processes:
        step_ids = set(_targets_of_type(graph, _outgoing(graph, process_id, "contains"), "process_step"))
        if not step_ids:
            findings.append(_finding("PROCESS_STEPS_MISSING", "error", "process contains no steps", process_id)); continue
        ...
        if not start_key:
            ...
            candidates = sorted(step_id for step_id in step_ids if step_id.endswith(f".{start_key}"))
            if not candidates:
...
def evaluate_adapter_graph(graph: Graph, kind: AdapterKind) -> dict[str, Any]:
    """Evaluate semantic conformance against adapter-contract/v0.1."""
    findings: list[dict[str, str]] = []
    expected_format = f"{kind}-as-code"
    if graph.project.get("source_format") != expected_format:
        findings.append(_finding("ADAPTER_SOURCE_FORMAT_MISMATCH", "warning", f"project.source_format is '{graph.project.get('source_format')}', expected '{expected_format}'"))
    if kind == "mapping": _mapping_conformance(graph, findings)
    elif kind == "interface": _interface_conformance(graph, findings)
    elif kind == "process": _process_conformance(graph, findings)
    else: raise GraphValidationError(f"unsupported adapter kind: {kind}")
    errors = sum(item["severity"] == "error" for item in findings)
    warnings = sum(item["severity"] == "warning" for item in findings)
    return {
        "contract": {"id": ADAPTER_CONTRACT_ID, "version": ADAPTER_CONTRACT_VERSION},
        "adapter": kind,
...
def should_fail_conformance(report: dict[str, Any], threshold: str) -> bool:
    ...
    if threshold == "warning": return bool(report["summary"]["errors"] or report["summary"]["warnings"])
    if threshold == "error": return bool(report["summary"]["errors"])
...
def render_conformance_markdown(report: dict[str, Any]) -> str:
    summary = report["summary"]; contract = report.get("contract", {})
    lines = [
    ...
    if not report["findings"]: lines.append("| — | — | — | No conformance findings |")
    return "\n".join(lines) + "\n"


def render_conformance_report(report: dict[str, Any], format: ConformanceFormat) -> str:
    if format == "json": return json.dumps(report, indent=2, ensure_ascii=False, sort_keys=True) + "\n"
    if format == "markdown": return render_conformance_markdown(report)
