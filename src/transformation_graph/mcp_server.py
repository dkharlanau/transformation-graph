...
def create_mcp_server(graph_path: str | Path) -> MCPServer:
    ...
    @mcp.tool()
    def governance_scorecard() -> dict:
        """Return transparent weighted governance coverage with explicit gaps."""
        return build_scorecard(graph)

    @mcp.tool()
    def traceability(source_type: str, target_type: str, max_depth: int = 4, directed: bool = False) -> dict:
        return traceability_matrix(graph, {source_type}, {target_type}, max_depth=max_depth, undirected=not directed)

    @mcp.tool()
    def role_view(role: str, max_depth: int = 4) -> dict:
        if role not in ROLE_PRESETS:
            return {"error": f"unknown role: {role}", "available_roles": sorted(ROLE_PRESETS)}
