...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Validate, compose, compare, and query Git-native enterprise transformation graphs.",
    )
    ...
    impact.add_argument("--direction", choices=["in", "out", "both"], default="both")
    impact.add_argument("--relation", action="append", dest="relations")
    ...
    mermaid.add_argument("file")
    mermaid.add_argument("--focus")
    ...
    import_csv.add_argument("--nodes", required=True)
    import_csv.add_argument("--edges", required=True)
    import_csv.add_argument("--project-id", required=True)
    import_csv.add_argument("--project-name", required=True)
    import_csv.add_argument("--description")
    ...
    compose.add_argument("files", nargs="+")
    ...
    diff.add_argument("--impact-depth", type=int, default=0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "import-csv":
            ...
            _json({"written": args.output, **graph.stats()})
            return 0
        ...
        if args.command == "compose":
            ...
            write_graph(graph, args.output)
            _json({"written": args.output, "sources": len(graphs), **graph.stats()})
            return 0

        if args.command == "diff":
            before = Graph.from_file(args.before)
            after = Graph.from_file(args.after)
            report = diff_with_impact(before, after, depth=args.impact_depth) if args.impact_depth > 0 else graph_diff(before, after)
            _json(report)
            return 0
        ...
        if args.command == "context":
            _json(graph.context(args.node, args.depth))
            return 0
        if args.command == "impact":
            _json(graph.impact(args.node, args.depth, args.direction, set(args.relations) if args.relations else None))
            return 0
        if args.command == "quality":
            report = graph.quality()
            _json(report)
            return 3 if args.strict and not report["passed"] else 0
        if args.command == "mermaid":
            print(graph.mermaid(args.focus, args.depth), end="")
            return 0
