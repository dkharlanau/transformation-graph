...
def test_mcp_server_registers_bounded_resources_and_tools():
    ...
    tools = asyncio.run(server.list_tools()); resources = asyncio.run(server.list_resources()); templates = asyncio.run(server.list_resource_templates())
    assert {tool.name for tool in tools} == {"get_context", "find_path", "analyze_impact", "graph_quality", "governance_scorecard", "governance_view", "traceability", "role_view"}
...
def test_mcp_context_resource_returns_existing_context():
    server = create_mcp_server(EXAMPLE)
    contents = asyncio.run(server.read_resource("transformation-graph://context/mapping.customer-to-bp")); items = list(contents)
    assert len(items) == 1
